...
export default class App extends React.Component {
    ...
    return this.state.dataSource.map(item => (
      ...
      >
        <View style={styles.partyContainer}>
          <Image source={partyImage[item.id]} />
  ...
  Header() {
    return (
      <View style={[styles.headerStyle]}>
        <Text style={[styles.textHeader]}>בחירות ישראל 2019</Text>
        <TouchableOpacity
          style={styles.button}
          onPress={() => {
  ...
  currScreen() {
    if (this.state.ScreenSwitcher === 'Results') {
      return <View style={styles.mainView}>{this.renderItemList()}</View>
    }
